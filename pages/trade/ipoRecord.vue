...
const rightClickHandle = () => {
    router.push('/record/ipoSuccessRecord')
}
const pub = usePublicStore()
const actRecordType = computed(() => {
  return pub.ipoActRecordType
})
...
<template>
    <div>
       <ClientOnly>
         <second-page-nav-bar :title="$t('trade.t78')" :right-icon="rightUrl"
      ...
      <div class="mt-3 px-3">
        <div class="recordBoxEl mt-3 pb-2">
          <div class="grid grid-cols-3 text-center recordTypeBoxEl">
            ...
            <div v-for="(item, index) in recordTypeList" :class="index == actRecordType ? 'actRecordEl' : ''"
              :key="index" @click="changeRecordTypeHandle(index)">
              {{ item }}
            </div>
