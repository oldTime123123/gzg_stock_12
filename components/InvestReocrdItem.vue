  ...
  <div class="mb-3 overflow-hidden rounded-2xl border border-[rgba(45,87,255,0.08)] bg-white shadow-[0_12px_28px_rgba(2,26,123,0.10)]">
    <div class="flex min-w-0 items-start justify-between gap-3 bg-[linear-gradient(135deg,#2D57FF_0%,#244ee6_100%)] px-4 py-3">
      <div class="min-w-0 flex-1">
        <span class="colorfff block break-words text-sm font-bold leading-[1.4]">{{ data.product.pro_name }}</span>
        <div class="flex">
          <div class="mt-2 h-5 max-w-full truncate rounded-full bg-[#FFDA1C] px-2 text-center text-xs leading-5 color000">
    ...
    <div class="grid gap-3 px-4 py-4 text-sm color000">
      <div class="grid gap-2 rounded-xl bg-[#f7f9ff] px-4 py-3">
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0 text-[#6c7690]">{{$t('comm.c41')}}</div>
          <div class="min-w-0 text-right font-semibold">{{ UseExchangeNumber(data.price)  }}</div>
        </div>
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0 text-[#6c7690]">{{$t('comm.c42')}}</div>
          <div class="min-w-0 text-right font-semibold" :class="getNumberClass(true, data.order_analysis?.is_rise)">
            {{ getNumberType(true, data.order_analysis?.is_rise) + data.order_analysis?.raise  }}
          </div>
        </div>
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0 text-[#6c7690]">{{$t('comm.c43')}}</div>
          <div class="min-w-0 text-right font-semibold" :class="getNumberClass(true, data.order_analysis?.is_rise)">
      ...
      <div class="flex items-center justify-center font-semibold text-[var(--mainColor)]" v-if="!data.showMore" @click="data.showMore = true">
        {{$t('comm.c44')}}
      ...
      <div class="grid gap-3" v-else>
        ...
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0 text-[#6c7690]">{{$t('comm.c45')}}</div>
          <div class="min-w-0 text-right font-semibold">{{ data.number }}</div>
        </div>
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0 text-[#6c7690]">{{$t('comm.c46')}}</div>
          <div class="min-w-0 text-right font-semibold">{{ UseExchangeNumber(data.price * data.number)  }}</div>
        </div>
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0 text-[#6c7690]">{{$t('comm.c47')}}</div>
          <div class="min-w-0 text-right font-semibold">{{ data.deal_time }}</div>
        </div>
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0 text-[#6c7690]">{{$t('comm.c49')}}</div>
          <div class="min-w-0 text-right font-semibold">{{ UseExchangeNumber(data.product?.price * data.number)  }}</div>
        </div>
        </div>
      ...
      <div class="grid grid-cols-2 gap-3 pt-1">
        <div class="contentBtn mt-0" @click="goDetails">{{$t('comm.c50')}}</div>
        ...
        <div class="w-[min(350px,calc(100vw-24px))] rounded-2xl bg-white p-4">
          ...
          <div class="mt-3 borderB pb-1">
            <div class="flex min-w-0 items-center gap-2">
              <span class="min-w-0 flex-1 break-words font-bold">{{ data.product.pro_name }}</span>
              <div class="ml-2 h-5 max-w-full truncate rounded-full bg-[#FFDA1C] px-2 text-xs leading-5 color000">
                {{ data.product.pro_code }}
              </div>
            </div>
            <div class="mt-3 grid gap-3">
            <div class="flex items-start justify-between gap-3">
              <div class="min-w-0 text-sm text-[#6c7690]">{{$t('comm.c53')}}</div>
              <div class="min-w-0 text-right font-semibold">
                {{ UseExchangeNumber(data.price)  }}
              </div>
            </div>
            <div class="flex items-start justify-between gap-3">
              <div class="min-w-0 text-sm text-[#6c7690]">{{$t('comm.c54')}}</div>
              <div class="min-w-0 text-right font-semibold">
                {{ UseExchangeNumber(data.product?.price)  }}
              </div>
            </div>
            <div class="flex items-start justify-between gap-3">
              <div class="min-w-0 text-sm text-[#6c7690]">{{ $t('record.a2') }}</div>
              <div class="min-w-0 text-right font-semibold">{{ data.number }}</div>
            </div>
            <div class="flex items-start justify-between gap-3">
              <div class="min-w-0 text-sm text-[#6c7690]">{{$t('comm.c55')}}</div>
              <div class="min-w-0 text-right font-semibold" :class="getNumberClass(true,data.profit_type)">
